    ...
    for file_name in helen_annotations_filelist:
        with open(file_name, 'r') as file:
            ...
            if len(face_position(gray_image)) != 1:
                print(f"Image includes more than one face: ignore {img_filename}")
            else:
...
def face_position(gray_img):
    ...
    """
    faces = face_cascade.detectMultiScale(gray_img, minSize=(int(len(gray_img[0]) / 6), int(len(gray_img) / 6)))
    return faces
